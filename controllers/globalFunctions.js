...
module.exports.registrationValidation = function(req, res) {
  ...
  req.checkBody({
    ...
    "lastName": {
      ...
      isLength: {
        options: [{min: 2, max: 50}],
        errorMessage: "Last name must be between 2 and 50 characters long." // takes precedent over parameter errorMessage
      },
      matches: {
        options: [/^[A-Za-z,.'-]+$/],
        errorMessage: "First name can only contain commas, dots, " +
    ...
    "email": {
      ...
      isEmailAvailable: {
        errorMessage: "Email address is not available. Please chooe another one."
      },
